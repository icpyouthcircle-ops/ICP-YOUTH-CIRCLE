...
(async () => {
  ...
  try {
    ...
    assert.equal((await page.locator('#mdcatBreadcrumb').innerText()).replace(/\s+/g,' ').trim(),'← Home / Entry Tests / MDCAT 2027');
    ...
    assert.equal((await page.locator('#mdcatBreadcrumb').innerText()).replace(/\s+/g,' ').trim(),'← Home / Entry Tests / MDCAT 2027 / Biology');
    ...
    assert.equal((await page.locator('#mdcatBreadcrumb').innerText()).replace(/\s+/g,' ').trim(),'← Home / Entry Tests / MDCAT 2027 / Biology / Demo Biology Unit');
    ...
    for (const failure of ['http','wrong-unit','wrong-subject']) {
      await page.locator('#mdcatBreadcrumb').getByRole('button',{name:'Biology',exact:true}).click();
      chapterMode = failure;
    ...
    chapterMode = 'success';
    await page.locator('#mdcatBreadcrumb').getByRole('button',{name:'MDCAT 2027',exact:true}).click();
